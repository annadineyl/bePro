...
function addTodo(value) {
    ...
    data.todo.push(value);  // Add task to the todo property in data object
    dataUpdated();
}
...
function editTodo(e) {
    ...
    function update() {
        if (editInput.value) {
        // If there is any value in edit input, then update the item
        ...
        // If there is no value in edit input, then remove the item
    ...
    // Edit and update task by clicking edit button
    if (e.type === 'click') {
...
function addTodoToDOM(txt, completed) {
    ...
    list.insertBefore(item, list.childNodes[0]);  // Insert the item before the first child of the list
}